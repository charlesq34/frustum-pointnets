...
import numpy as np
sys.path.append(os.path.join(BASE_DIR, '../../kitti_data'))
from utils import roty
sys.path.append('../../')
...
class ROISegBoxDataset(object):
    def __init__(self, npoints, split, random_flip=False, random_shift=False, rotate_to_center=False, overwritten_data_path=None, from_rgb_detection=False, one_hot=False):
        ...
        if overwritten_data_path is None:
            overwritten_data_path = os.path.join(BASE_DIR, '../../kitti_data/roi_seg_box3d_carpedcyc_%s_rqi_1031.pickle'%(split))
    ...
    def __getitem__(self, index):
        ...
        if self.random_flip:
            if np.random.random()>0.5:
                ...
                # TODO: rot_angle won't be correct if we have random_flip...
...
if __name__=='__main__':
    ...
    median_list = []
    dataset = ROISegBoxDataset(2048, split='val', rotate_to_center=True, random_flip=True, random_shift=True)
    for i in range(len(dataset)):
        data = dataset[i]
        print 'Center: ', data[2], 'angle_class: ', data[3], 'angle_res:', data[4], 'size_class: ', data[5], 'size_residual:', data[6], 'real_size:', g_type_mean_size[class2type[data[5]]]+data[6] 
